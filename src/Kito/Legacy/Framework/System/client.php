...
function getlocation(){$URL=$_SERVER["REQUEST_URI"];return getDomain().substr($URL,1,strpos($URL, "/", 1));}
function getCookieName($Name){return "BLK_".md5($Name.crc32(GetValue("Title","BLK Application")));}
function putCookie($Name,$Value){return setcookie(getCookieName($Name),$Value,0,"/");}
function delCookie($Name){putCookie($Name,"");}
function getCookie($Name){return (isset($_COOKIE[getCookieName($Name)]))?$_COOKIE[getCookieName($Name)]:false;}
  ...
  function getIP()
    ...
    function getBrowser(){return (isset( $_SERVER['HTTP_USER_AGENT'] ) ) ? strtolower( $_SERVER['HTTP_USER_AGENT'] ) : false;}
    function isIE(){return stristr(getBrowser(), "msie");}
